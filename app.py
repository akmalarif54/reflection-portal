...
if 'tasbih_phrase' not in st.session_state: st.session_state.tasbih_phrase = "SubhanAllah"
if 'cal_month_offset' not in st.session_state: st.session_state.cal_month_offset = 0
if 'water_ml' not in st.session_state: st.session_state.water_ml = 0
if 'current_hadith_idx' not in st.session_state: st.session_state.current_hadith_idx = datetime.now().day

# --- THEME STYLING WITH FULL VISIBILITY FIXED ---
st.markdown("""
    <style>
    .stApp { background-color: #FDFBF7 !important; color: #2C2520 !important; font-family: 'Inter', sans-serif; }
    *::selection, * *::selection, .stApp *::selection, div *::selection { background: #F7F3EB !important; color: #A3843B !important; }
    *::-moz-selection, * *::-moz-selection, .stApp *::-moz-selection, div *::-moz-selection { background: #F7F3EB !important; color: #A3843B !important; }
    
    /* Gilded Frame Card Container */
    .islamic-card { background-color: #FFFFFF; padding: 30px; border-radius: 20px; border-top: 5px solid #D4AF37; box-shadow: 0 4px 20px rgba(212, 175, 55, 0.08); margin-bottom: 25px; position: relative; }
    ...
    /* Fixed Dark Background Code Block Override */
